...
parser = argparse.ArgumentParser("pubmed scrapper")
parser.add_argument("-k", "--keyword", nargs='+', help="keyword split by space", type=str)
parser.add_argument("-n", "--number", nargs='?', default=200, help="retrieving number of results", type=int)
args = parser.parse_args()

# retrieve keyword from arg
...
# takes the soup and extracts all needed elements for the bibliography and abstract
...
def get_bibliography(soup):
    ...
    # creates a empty variable for each needed element and subsequently fills in the true value if it exists
...
# loop over each element in the idlist to get the soup and feed it into our function
for link in idlist:
